...
solangapsukien18++;
    for(int i=0;i<n9;i++){
        if(chuoi_merlin[i]=="Merlin" ||chuoi_merlin[i]=="merlin" ) {   cout<<"HP cong 3 va tang den maxhp"<<endl;
            HP+=3;
            HP=HP>MaxHP?MaxHP:HP;
        }
        else { cout<<"HP cong 2 va tang den maxhp"<<endl;    HP+=2;HP=HP>MaxHP?MaxHP:HP;}
    }
}
